...
export class CRUDService {
	...
	/**
	 * Returns latest block Creation Time.
	 * @param {*} channel_genesis_hash
	 * @returns
	 * @memberof CRUDService
	 */
	async getBlockByCreationTime(network_name: any, channel_genesis_hash: any) {
		const creationTime = await this.sql.getRowByPkOne(
			'select max(createdt) from blocks where channel_genesis_hash = $1 and network_name = $2',
			[channel_genesis_hash, network_name]
		);
		return creationTime;
	}
